...
// MIME multipart parsing, as defined in RFC 2046.

// Originally from go/src/mime/multipart/multipart.go
...
func (p *Part) parseContentDisposition() {
	...
	var err error
	p.disposition, p.dispositionParams, err = mime.ParseMediaType(string(v))
	if err != nil {
...
var quotedPrintable = []byte("quoted-printable")
...
func newPart(mr *MultipartReader) (*Part, error) {
	...
	if bytes.Equal(bp.Header.Get(cte), quotedPrintable) {
		bp.Header.Del(cte)
