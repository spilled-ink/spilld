...
// Originally from go/src/mime/multipart/multipart_test.go
...
func TestNameAccessors(t *testing.T) {
	...
	for i, test := range tests {
		p := &Part{Header: email.Header{}}
		p.Header.Add("Content-Disposition", []byte(test[0]))
...
func testMultipart(t *testing.T, r io.Reader, onlyNewlines bool) {
	...
	if x := string(part.Header.Get("Header1")); x != "value1" {
		t.Errorf("part.Header.Get(%q) = %q, want %q", "Header1", x, "value1")
	}
	if x := string(part.Header.Get("Foo-Bar")); x != "baz" {
	...
	if e, g := "bigsection", string(part.Header.Get("Name")); e != g {
	...
	if string(part.Header.Get("Foo-Bar")) != "bazb" {
...
`
	...
	var i int
	readPart := func(hdr email.Header, body string) {
		part, err := reader.NextPart()
		if part == nil || err != nil {
	...
	readPart(mkHeader("Foo-Bar", "baz"), "Body")
	...
	readPart(mkHeader("Foo-Bar", "bop"), "Body 2")
}
...
func TestQuotedPrintableEncoding(t *testing.T) {
	...
	r := NewMultipartReader(strings.NewReader(body), "0016e68ee29c5d515f04cedf6733")
	part, err := r.NextPart()
	if err != nil {
		t.Fatal(err)
	}
	if te, ok := part.Header.Index["Content-Transfer-Encoding"]; ok {
...
func TestNested(t *testing.T) {
	...
	defer f.Close()
	mr := NewMultipartReader(f, "e89a8ff1c1e83553e304be640612")
	p, err := mr.NextPart()
	if err != nil {
...
func mkHeader(el ...string) (h email.Header) {
	h.Index = make(map[email.Key][][]byte)
	for i := 0; i < len(el); i += 2 {
		key := email.Key(el[i])
		val := []byte(el[i+1])
		h.Entries = append(h.Entries, email.HeaderEntry{Key: key, Value: val})
		h.Index[key] = append(h.Index[key], val)
	}
	return h
}
...
------=_NextPart_4c2fbafd7ec4c8bf08034fe724b608d9--`, "\n", "\r\n", -1),
		want: []headerBody{
			{mkHeader("Content-Type", `multipart/alternative; boundary="----=_NextPart_4c2fbafd7ec4c8bf08034fe724b608d9_alt"`),
